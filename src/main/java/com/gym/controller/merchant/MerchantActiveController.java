...
@RestController
@RequestMapping("/merchant/active")
@Slf4j
public class MerchantActiveController {
    @Resource
    ...
    public AjaxResult<IPage<FcGymActiveBean>> getPage(int current, int size, HttpServletRequest request) {
        try {
            Page<FcGymActiveBean> page = new Page<>(current, size);
            return AjaxResult.success(gymActiveService.userPageVo(page, request), "查询成功");
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public AjaxResult update(@RequestBody FcGymActive fcGymActive) {
        try {
            gymActiveService.updateActive(fcGymActive);
    ...
    public AjaxResult save(@RequestBody FcGymActive fcGymActive) {
        try {
            gymActiveService.saveActive(fcGymActive);
    ...
    public AjaxResult delete(@PathVariable Integer id) {
        try {
            gymActiveService.deleteActive(id);
